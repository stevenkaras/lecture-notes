...
% I got a blast of text messages about a server that crashed at work and was not able to keep track of what was going on.
...
% I got yet another blast of messages about the crashed server. I caught the gist of it, and I'll fill in the details later

\paragraph{Bidirectional variant}

% gets the two neighbors and makes a local decision about it being the leader or not. If not, turns into a relay.
% repeat until only one remains

\paragraph{Unidirectional algorithm}

% get two prior neighbors. Makes the same local decision as before, but assumes the label of the previous neighbor.
% repeat until only one remains
% basic approach shifts decision to another node.

% In order to handle the case of two processes, a process replaces an id lower than its own with itself (so 8 will replace 5 with 8 to send on)
